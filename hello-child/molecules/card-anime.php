...
<div class="card-anime <?php echo $class; ?>">
	...
	<a href="<?php echo $url; ?>" class="card-anime__media" aria-label="<?php echo esc_attr( sprintf( __( 'Ver detalhes do anime %s', 'hello-elementor-child' ), $titulo ) ); ?>">
		<?php 
		mm_render_component( 'atoms', 'imagem-capa', array(
			'src'          => $imagem_url,
			'alt'          => sprintf( __( 'Capa do anime: %s', 'hello-elementor-child' ), $titulo ),
	...
	<div class="card-anime__content">
		...
		<?php if ( ! empty( $generos ) && is_array( $generos ) ) : ?>
			<div class="card-anime__genres" role="list" aria-label="<?php esc_attr_e( 'Gêneros do anime', 'hello-elementor-child' ); ?>">
				<?php 
